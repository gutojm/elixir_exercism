defmodule FileSniffer do
  ...
  def verify(file_binary, extension) do
    {type_from_binary(file_binary), type_from_extension(extension)}
    |> do_verify()
  end

  defp do_verify({fb,ex}) when fb == ex, do: {:ok, fb}
  defp do_verify({fb,ex}) when fb != ex, do: {:error, "Warning, file format and file extension do not match."}
end
